...
import unittest
class RandomCS:
    def __init__(self, n, hash, degree):
        # Code block length
        assert (hash.n == n)
        self.n = n
        self.degree = degree
        ...
        self.no_binary_measurements = RandomCS._get_number_measurements(n, degree)
    ...
    def _get_number_measurements(n, d):
        return ceil(np.log2(sum([RandomCS._nCr(n,i) for i in range(0,d+1)]))) +1
