        ...
        for ratio in np.linspace(1.1,2.1,10):
            for try_number in range(10):
                path = Path(f"../results/reed_solomon/{dataset}_n={n}_no_trees={no_trees}_depth={depth}_"
                            f"C={C:.3}_ratio={ratio:.3}_tryno={try_number}.json")
                if not path.is_file():
                    submit_string = f"bsub -W {depth_to_time[depth]} "\
                                    f" -o logs/{dataset}_n={n}_no_trees={no_trees}_depth={depth}_" \
                                    f"C={C:.3}_ratio={ratio:.3}_tryno={try_number}.txt"\
                                    f" -R rusage[mem={depth_to_mem[depth]}] "\
                                    f"python -u reed_solomon_runner.py {n} {no_trees} {depth} {try_number} {C} {ratio} {dataset} "\
                                    f"&> /dev/null"
                    if not dry_run:
